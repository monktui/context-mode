...
function readSessionEvents(sessionId: string) {
...
describe("Pi Extension", () => {
	...
	describe("Slice 2: Event extraction from tool_result", () => {
		...
		it("normalizes Pi path params for read/edit telemetry", async () => {
			await registerPiExtension(api);
			const sessionId = await startSession(join(tempDir, "read-edit.jsonl"));
			...
			await api._trigger("tool_result", {
				tool_name: "edit",
				tool_input: { path: "/src/index.ts", oldText: "hello", newText: "hi" },
				tool_result: "File edited",
			});

			const events = readSessionEvents(sessionId);
			expect(events.find((e) => e.type === "file_read")?.data).toBe(
				"/src/index.ts",
			);
			expect(events.find((e) => e.type === "file_edit")?.data).toBe(
				"/src/index.ts",
			);
		});

		it("captures Pi error text and raw breadcrumb", async () => {
			await registerPiExtension(api);
			const sessionId = await startSession(join(tempDir, "error.jsonl"));
			...
			const events = readSessionEvents(sessionId);
			const error = events.find((e) => e.type === "error_tool");
			expect(error?.data).toContain("npm test");
			expect(error?.data).toContain("test failed");
			expect(events.find((e) => e.type === "tool_call")?.data).toContain(
				"npm test",
			);
		});
