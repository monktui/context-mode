...
describe("Plan Mode Events", () => {
	...
	test("extracts plan_file_write from Write to ~/.claude/plans/", () => {
		const input = {
			tool_name: "Write",
			tool_input: {
				file_path: "/Users/test/.claude/plans/jaunty-nebula.md",
				content: "# Plan",
			},
			tool_response: "ok",
		};
...
describe("Role Events", () => {
	test("extracts role from persona directive", () => {
		const events = extractUserEvents(
			"Act as a senior staff engineer for this review",
		);
	...
	test("extracts role from 'you are' pattern", () => {
		const events = extractUserEvents(
			"You are a principal architect. Review this design.",
		);
		const roleEvents = events.filter((e) => e.type === "role");
		assert.equal(roleEvents.length, 1);
	});
...
describe("Subagent Events", () => {
	...
	test("completed agent event type indicates completion status", () => {
		...
		assert.ok(
			subagentEvents[0].type.includes("completed") ||
				subagentEvents[0].type.includes("complete"),
...
describe("Intent Events", () => {
	test("extracts investigation intent", () => {
		const events = extractUserEvents(
			"Why is the test failing? Can you debug this?",
		);
	...
	test("extracts implementation intent", () => {
		const events = extractUserEvents(
			"Create a new PostToolUse hook for event extraction",
		);
		const intentEvents = events.filter((e) => e.type === "intent");
		assert.equal(intentEvents.length, 1);
	...
	test("extracts review intent", () => {
		const events = extractUserEvents(
			"Review this code and check for security issues",
		);
		const intentEvents = events.filter((e) => e.type === "intent");
		assert.equal(intentEvents.length, 1);
	...
	test("extracts discussion intent", () => {
		const events = extractUserEvents(
			"Think about the pros and cons of this approach",
		);
		const intentEvents = events.filter((e) => e.type === "intent");
		assert.equal(intentEvents.length, 1);
...
describe("AskUserQuestion Events", () => {
	test("extracts decision_question event from AskUserQuestion", () => {
		...
		assert.ok(
			decisionEvents[0].data.includes("database"),
...
describe("EnterWorktree Events", () => {
	test("extracts worktree event from EnterWorktree", () => {
		...
		assert.ok(
			wtEvents[0].data.includes("feature-auth"),
			"should include worktree name",
		);
	});
	...
	test("extracts worktree event without name", () => {
		...
		assert.equal(wtEvents.length, 1);
		assert.ok(
			wtEvents[0].data.length > 0,
			"should have data even without name",
		);
	});
});
...
describe("New Git Patterns", () => {
	test("extracts git add event", () => {
		...
		assert.equal(gitEvents.length, 1);
		assert.ok(
			gitEvents[0].data.includes("add"),
			"should include add operation",
		);
	});
	...
	test("extracts git cherry-pick event", () => {
		...
		assert.equal(gitEvents.length, 1);
		assert.ok(
			gitEvents[0].data.includes("cherry-pick"),
			"should include cherry-pick",
		);
	});
...
describe("New Env Patterns", () => {
	test("extracts env event from cargo install", () => {
		...
		assert.ok(
			envEvents.length >= 1,
			"should extract env event for cargo install",
		);
	});
	...
	test("extracts env event from deno install", () => {
		...
		assert.ok(
			envEvents.length >= 1,
			"should extract env event for deno install",
		);
	});
});
...
describe("Env Secret Sanitization", () => {
	test("sanitizes export commands to prevent secret leakage", () => {
		...
		assert.ok(
			!envEvents[0].data.includes("sk-secret"),
			"should NOT contain the secret value",
		);
		assert.ok(
			envEvents[0].data.includes("API_KEY"),
	...
	test("does not sanitize non-export env commands", () => {
		...
		assert.ok(envEvents.length >= 1, "should extract env event");
		assert.ok(
			envEvents[0].data.includes("npm install express"),
			"should contain full command",
		);
	});
});
...
describe("Safety — safeString preserves full data", () => {
	test("preserves full tool response in error events (no truncation)", () => {
		...
		const errorEvents = events.filter((e) => e.type === "error_tool");
		assert.equal(errorEvents.length, 1);
		const payload = JSON.parse(errorEvents[0].data);
		assert.equal(
			payload.error,
			longError,
			"Full error string must be preserved",
		);
		assert.equal(
			payload.response,
			longError,
			"Full response must be preserved",
		);
		assert.equal(payload.input.command, "npm test");
	});
	...
	test("data field is always a string and preserves full content", () => {
		...
		for (const event of events) {
			assert.equal(
				typeof event.data,
				"string",
				`event.type=${event.type} data should be string`,
			);
		}
		assert.equal(
			events[0].data,
			longPath,
			"Full path must be preserved without truncation",
		);
	});
});
...
describe("Glob Events", () => {
	test("extracts file_glob event from Glob tool call", () => {
		const input = {
			...
			tool_response: JSON.stringify({
				filenames: ["src/server.ts", "src/runtime.ts"],
			}),
		};
...
describe("Zero-truncation architecture", () => {
	...
	test("safeString preserves full data without truncation", () => {
		...
		assert.equal(
			events[0].data,
			longPath,
			"safeString must preserve full string without truncation",
		);
	});
...
describe("MCP Events", () => {
	test("captures tool_response content so ctx_search can find details, not just the call", () => {
		const input = {
			...
			tool_response: JSON.stringify({
				...
				description:
					"Agents see intermittent disconnects during broker failover...",
			}),
		};
		...
		assert.ok(
			events[0].data.includes("jira_get"),
			"data should include tool short name",
		);
		assert.ok(
			events[0].data.includes("CVX-5909"),
			"data should include first string arg",
		);
	...
	test("gracefully handles empty tool_response", () => {
		...
		assert.equal(
			events[0].data,
			"ctx_stats",
			"empty tool_response should not add suffix",
		);
	});
});
...
describe("Agent Finding Events", () => {
	test("extracts agent_finding when Agent completes with tool_response", () => {
		const input = {
			...
			tool_response:
				"Found 3 auth patterns: JWT, OAuth2, and session-based. JWT is stateless...",
		};
...
describe("External Ref Events", () => {
	test("extracts URLs from tool_input", () => {
		...
		assert.ok(
			refs[0].data.includes("https://api.github.com"),
			"should capture URL",
		);
	});
	...
	test("captures multiple mixed refs", () => {
		const input = {
			...
			tool_response:
				"Found https://github.com/user/repo/pull/100 and #344 is related to https://docs.example.com/guide",
		};
		...
		assert.ok(
			refs[0].data.includes("github.com/user/repo/pull/100"),
			"should include PR URL",
		);
		assert.ok(
			refs[0].data.includes("docs.example.com"),
			"should include doc URL",
		);
	});
});
...
describe("Category 27 — Latency", () => {
	...
	test("extractEvents does not produce latency events (latency is cross-hook, not extract-based)", () => {
		...
		assert.equal(
			...
			0,
			"extractEvents should not produce latency events",
		);
	});
...
describe("Category 28 — Permission (architectural limitation)", () => {
	...
	test("PostToolUse always implies approval — no deny signal available", () => {
		...
		assert.equal(
			permEvents.length,
			0,
			"no permission events should be extracted — not implementable",
		);
	});
	...
	test("PreToolUse deny is already captured as rejected-approach (Phase 1)", () => {
		...
		const rejectedEvents = events.filter(
			(e) => e.category === "rejected-approach",
		);
		assert.equal(
			rejectedEvents.length,
			0,
			"extractEvents does not produce rejected-approach — that's pretooluse.mjs's job",
		);
	});
});
...
describe("Error Resolution Events", () => {
	test("emits error_resolved when error followed by successful same-tool call", () => {
		...
		assert.equal(
			errors.length,
			0,
			"should not emit error_resolved on the error itself",
		);
	...
	test("emits error_resolved when Edit follows Read error", () => {
		...
		assert.ok(
			resolved[0].data.includes("Read"),
			"should reference Read as the error tool",
		);
	});
	...
	test("clears lastError after 10 calls without resolution (staleness timeout)", () => {
		...
		assert.equal(
			resolved.length,
			0,
			"stale error should be cleared after 10 calls",
		);
	});
...
describe("Iteration Loop Events", () => {
	test("emits retry_detected when same tool+input called 3 times consecutively", () => {
		...
		assert.equal(
			retries.length,
			1,
			"should fire after 3 consecutive identical calls",
		);
		...
		assert.ok(
			retries[0].data.includes("suggestion"),
			"should include optimization guidance",
		);
	});

	test("keeps detecting todo loops with actionable batching guidance", () => {
		resetIterationLoopState();

		const input = {
			tool_name: "todo",
			tool_input: { action: "update", id: 1, status: "done" },
			tool_response: "ok",
		};
		...
		assert.ok(retries[0].data.includes("todo"));
		assert.ok(retries[0].data.includes("Batch adjacent task/status updates"));
	});
	...
	test("does not emit when different tools are interleaved", () => {
		...
		assert.equal(
			retries.length,
			0,
			"interleaved tools should break the streak",
		);
	});
	...
	test("detects 4+ consecutive identical calls", () => {
		...
		assert.ok(
			retries.length === 0,
			"4th call after reset should not emit (fresh start)",
		);
	});
	...
	test("resets after emission to avoid duplicate events", () => {
		...
		assert.equal(
			retries.length,
			0,
			"should not emit at 2nd call of new streak",
		);
...
describe("Blocked-On Events", () => {
	...
	test("extracts blocker from 'can't proceed until' pattern", () => {
		const events = extractUserEvents(
			"can't proceed until the migration is done",
		);
		const blockerEvents = events.filter((e) => e.type === "blocker");
		assert.equal(blockerEvents.length, 1);
	});
	...
	test("extracts blocker from 'depends on' pattern", () => {
		const events = extractUserEvents(
			"this depends on the auth service being up",
		);
		const blockerEvents = events.filter((e) => e.type === "blocker");
		assert.equal(blockerEvents.length, 1);
	});
	...
	test("extracts blocker_resolved from 'unblocked' pattern", () => {
		const events = extractUserEvents(
			"we're unblocked now, got the credentials",
		);
	...
	test("resolution takes priority over blocker when both match", () => {
		...
		assert.equal(
			...
			0,
			"should NOT emit blocker when resolved",
		);
	});
